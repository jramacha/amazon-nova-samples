...
io.on('connection', (socket) => {
    ...
    socket.on('conversationResumption', async () => {
            try {
                const session = socketSessions.get(socket.id);
                const currentState = sessionStates.get(socket.id);
                console.log(`Resume conversation for Don Smith, check in date is 2025-05-15 for ${socket.id}, session exists: ${!!session}, state: ${currentState}`);
                ...
                if (!session) {
                    console.error(`No session found for conversation history: ${socket.id}`);
                    socket.emit('error', { message: 'No active session for conversation history' });
                    return;
                } 
                ...
                socket.emit('error', {
                    message: 'Error processing conversation history',
                    details: error instanceof Error ? error.message : String(error)
                });
            }
        });
